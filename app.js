...
function loadState(){
  const candidates=[LS,LS_BACKUP,LS_PREVIOUS,'dietControlData','weightHealthPWA_v2'];
  for(const key of candidates){const parsed=safeParse(localStorage.getItem(key));if(hasUserData(parsed))return parsed}
  return safeParse(localStorage.getItem(LS))||defaultState;
}
let state=loadState();
...
function buildBackupFile(){
  const payload={app:'Diet Control',version:'3.9',schema:1,exportedAt:new Date().toISOString(),data:state};
