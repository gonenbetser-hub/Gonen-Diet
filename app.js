...
function renderAll(){renderToday();renderActions();renderMetrics();renderMeals();renderActivities();renderHealth();renderReports();renderEvidence();fillSettings();bindInfo()}
function renderToday(){const m=latestMetric(),c=calc(m),sc=score();q('todayWeight').textContent=m.weight?`${(+m.weight).toFixed(1)} ק״ג`:'—';q('goalWeight').textContent=`${state.profile.targetWeight||'—'} ק״ג`;
 ...
 const td=today(),meals=state.meals.filter(x=>x.date===td),acts=state.activities.filter(x=>x.date===td),eaten=sum(meals,'calories'),prot=sum(meals,'protein'),burn=sum(acts,'calories'),target=+state.profile.calorieTarget||0,rem=target-eaten,net=eaten-burn;q('calTarget').textContent=target;q('calEaten').textContent=eaten;q('calBurned').textContent=burn;q('calNet').textContent=net;q('calorieRemaining').textContent=rem;const pct=target?Math.min(100,eaten/target*100):0;q('calorieRing').style.background=`conic-gradient(#111827 ${pct}%,#e8eaee ${pct}%)`;const pt=+state.profile.proteinTarget||0,pp=pt?Math.min(100,prot/pt*100):0;q('proteinProgress').style.width=pp+'%';q('proteinPct').textContent=Math.round(pp)+'%';q('proteinText').textContent=`${prot} / ${pt} גרם`}
